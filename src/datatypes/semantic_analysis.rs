...
impl<'a> SemanticAnaytis<'a> {
    ...
    pub fn process_statement(&mut self, statement : &'_ CgStatement) -> Result<CgStatement, String> {
        match statement.statement_type.clone() {
            CgStatementType::VariableInitialization(var_init) => {
                ...
                let init_valid = match (var_in_stack_frame.variable_type, var_init.init_value) {
    ...
    pub fn process_stack_frame(&mut self, stack_frame : usize) -> StackFrame {
        let mut processed_stack_frame = StackFrame::new(9);

        processed_stack_frame.statements.clear();
        
        for statement in stack_frame.statements.iter() {
            let analyzed_statement = self.process_statement(statement);

            match analyzed_statement {
                Ok(successfull_statement) => {
                    processed_stack_frame.statements.push(successfull_statement);
                },
                Err(err) => {
                    if err.is_empty() {
                        continue;
